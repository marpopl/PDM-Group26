...
def runner(start_pos = np.array([0.0, 20.0, 0.0]), goal_pos = np.array([.0, -25.0, 0.0]), obstacles=None, region=5, path='a_star',visualise=True):
    ...
    if path == 'a_star':
        ...
        length_path = calculate_path_length(a_star_path)
...
def calculate_path_length(path):
    ...
    Parameters:
        path (list of tuples or ndarray): The path as a list of (x, y) points.

    Returns:
        float: The total length of the path.
    """
    if len(path) < 2:
        return 0.0  # A single point or no path has zero length
