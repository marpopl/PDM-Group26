    ...
    while open_set:
        ...
        for steering in steering_angles:
            ...
            for _ in range(steps):
                ...
                # Check collision for every step
            ...
            if collision_free:
                all_end_states.append(trajectory[-1].copy())  # Append only the final state
...
def generate_path_with_model(model, start_pos, goal_pos, obstacles, car_size, velocity=1.0, dt=1.0, max_depth=3):
    ...
    while open_set:
        ...
        for i, end_state in enumerate(end_states):
            ...
            if end_tuple not in cost_so_far or new_cost < cost_so_far[end_tuple]:
    ...
    # Path reconstruction: Append all intermediate states in correct order
    path = []
    control_list = []
    curr = goal_tuple

    while curr != start_tuple:
        if curr in trajectories:
            trajectory = trajectories[curr]
            control_trajectory = control_trajectories[curr]
            
            # Avoid duplicates: only append if trajectory is new
            if not path or not np.array_equal(trajectory[-1], path[0]):
                path = trajectory[:-1] + path  # Append trajectory, excluding the final node to avoid duplicates
                control_list = control_trajectory + control_list
    ...
    path = [start_pos] + path  # Ensure the start position is included
    all_expanded_states = [state for traj in trajectories.values() for state in traj]

    return path, control_list, all_expanded_states
